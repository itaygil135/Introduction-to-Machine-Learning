...
class Perceptron(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        - At the end of each iteration the `self.callback_` function is called passing the current
          status of the object (index.e self), the current misclassified sample and the label of the
          current misclassified sample. The function is called once more after the fitting is completed
        ...
        for _ in range(self.max_iter_):
            ...
            index = next((index for index in range(len(X)) if y[index] * (X[index] @ self.coefs_) <= 0), None)
            if index is None:
                break
