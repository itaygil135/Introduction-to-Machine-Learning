...
class LDA(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        self.cov_ = np.einsum("ki,kj->kij", c, c).sum(axis=0) / (len(X)-len(self.classes_))
    ...
    def likelihood(self, X: np.ndarray) -> np.ndarray:
        ...
        a1 = np.sqrt((2*np.pi) ** X.shape[1] * np.linalg.det(self.cov_))
        a2 = X[:, np.newaxis, :] - self.mu_
        a3 = np.exp(-.5 * np.sum(a2.dot(self.cov_inv_) * a2, axis=2)) / a1
        return a3 * self.pi_
