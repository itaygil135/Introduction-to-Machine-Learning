...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        l_reg = np.random.normal(0, 1, X.shape[1]) / np.sqrt(X.shape[1])
        ...
        if self.penalty_ != "none":
            if self.penalty_ != "l1":
                ...
                pattern_of_reg = L1()
            estimator = RegularizedModule(LogisticModule(l_reg), pattern_of_reg, self.lam_, l_reg,
                                          self.include_intercept_)

        else:
            estimator = LogisticModule(l_reg)

        self.coefs_ = self.solver_.fit(estimator, X, y)
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.include_intercept_:
            X = np.hstack((np.ones((X.shape[0], 1)), X))
        wsAtExp = np.exp(np.dot(X, self.coefs_))
        return wsAtExp / (wsAtExp + 1)
    ...
    def _loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        McfError = misclassification_error(y, self._predict(X))
        return McfError
