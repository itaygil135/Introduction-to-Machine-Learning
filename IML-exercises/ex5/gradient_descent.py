...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        solution = f.weights_
        iter_num = 0
        top_value = f.compute_output(X=X, y=y)
        top_weights_solution = solution
        average = 0
        length = self.max_iter_
        put = 0

        while iter_num < length:
            eta = self.learning_rate_.lr_step(t=iter_num)
            current_value_gradient = f.compute_jacobian(X=X, y=y)
            new_sol = solution - eta * current_value_gradient
            delta = linalg.norm(solution - new_sol)

            solution = new_sol  # param next
            f.weights = new_sol

            current_value = f.compute_output(X=X, y=y)
