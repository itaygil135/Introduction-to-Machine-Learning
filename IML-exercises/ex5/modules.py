...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        z = X @ self.weights
        output = np.mean(np.log(1 + np.exp(z)) - y * z)
        return -output
...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        return np.add(self.regularization_module_.compute_output(**kwargs) * self.lam_,
                      self.fidelity_module_.compute_output(**kwargs))
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        """
        reg_output = self.regularization_module_.compute_output(**kwargs)
        fid_output = self.fidelity_module_.compute_output(**kwargs)
        if self.include_intercept_:
            reg_output *= self.lam_
        else:
            reg_output *= self.lam_ * np.concatenate(([0], np.ones(self.weights.shape[0] - 1)))
        return np.add(fid_output, reg_output)
    ...
    def weights(self, weights: np.ndarray) -> None:
        ...
        self.regularization_module_.weights = np.delete(weights, 0, axis=0) if self.include_intercept_ else weights
